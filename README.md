...
一个面向普通用户的桌面应用：输入当天总结内容，选择或自定义主题与模块，一键生成高质感的海报图片（PNG/JPG）。默认提供四个高端配色主题、模板库与多种模块，支持模块级样式（含渐变）与主题调色板。应用可离线运行，无需编程知识。
...
- 模块化编辑：
  ...
  - 新增：自定义模块（Rich）与图片/贴纸模块。
  - 支持添加/删除/排序模块，自由组合。
- 主题与配色：
  - 内置四个「高端」主题：极简黑金、奶油莫兰迪、薄雾蓝紫、清爽夏日。
  - 主题调色板编辑器：可自定义主题颜色与背景渐变（0/90°）。
  - 模块级样式（module-independence）：每个模块可单独设置卡片纯色/渐变、文本色、强调色、圆角与内边距。
- 版式与排版：
...
- 体验与易用性：
  - 所见即所得预览、主题一键切换、颜色选择器与渐变选择。
  - 文件菜单：新建/打开/保存/另存为；从模板新建。
  - 离线运行、本地 JSON 模板文件。
- 安全与隐私：
  - 完全本地处理，不上传任何数据。

新增特性（当前版本）
- 模板库：`examples/templates` 提供多套可直接使用的模板
- 背景与卡片渐变：支持横向/纵向线性渐变
- 自定义模块（富文本）：标题/正文/列表，可插图，支持 emoji
- 图片/贴纸模块：加载本地图片，cover/contain，固定高度卡片
- 模块级样式：单模块独立色彩与圆角、内边距
...
- 自定义（`rich`）：标题/正文/列表、可选图片，支持对齐与样式覆写。
- 图片/贴纸（`image`）：插入本地图片，支持 cover/contain，固定高度卡片。
...
- 推荐 Python 3.11
- Conda 环境（含 Tk）：
  - `conda create -n dspg-py311 python=3.11 tk -y`
  - `conda activate dspg-py311`
...
### 6.3 模块级样式（module-independence）
每个模块支持 `style` 字段，覆盖主题默认值：
```
{
  "type": "summary",
  "title": "今日清单",
  ...
  "style": {
    "bg_color": "#FFFFFF",
    "bg_gradient": { "start": "#FFDEE9", "end": "#B5FFFC", "angle": 90 },
    "text_color": "#222222",
    "accent_color": "#2BB673",
    "radius": 24,
    "padding": 28
...
### 6.4 主题内嵌（自定义主题导出）
保存时可将主题作为 `theme_data` 内嵌：
```
{
  "theme": "custom",
  "theme_data": {
    "palette": { "background": "#101522", "text": "#ECEFF4", "primary": "#7A88FF", "accent": "#D0A2F7", "card": "#151a2b", "card_border": "#22273a" },
...
- v0.2 自定义：主题调色板、背景/卡片渐变、模块级样式、自定义模块与图片/贴纸、模板库、打开/保存配置。
- v0.3 体验：拖拽排序、2x/3x 导出、主题分享/导入、模块图库与贴纸包。
- v0.4 进阶：PDF 导出、富文本（加粗/高亮/链接）、对比度自动校正。
- v0.5 协作/智能：模板分享市场、命令行批处理、（可选）本地建议与文案增强。
...
- Q：无法联网环境下可用吗？
  - A：完全离线可用，资源全部本地。
- Q：emoji 显示不一致或不彩色？
  - A：受系统字体与 Pillow 支持影响，若不满意可用“图片/贴纸”模块插入贴图以保证视觉效果。
